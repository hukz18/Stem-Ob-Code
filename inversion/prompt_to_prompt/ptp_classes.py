...
import inversion.prompt_to_prompt.ptp_utils as ptp_utils
import inversion.prompt_to_prompt.seq_aligner as seq_aligner
import torch
...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0, prompts=None, tokenizer=None):
    ...
    for i in range(len(tokens)):
        ...
        image = ptp_utils.text_under_image(image, decoder(int(tokens[i]), clean_up_tokenization_spaces=True))
        images.append(image)
